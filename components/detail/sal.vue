...
export default {
  ...
  methods: {
    getHeading () {
      const headings = evaluateXPathToNodes('//sal/heading', this.doc)
    ...
    refreshData () {
      ...
      const ekuitasAkhir = [{
        account: 'Saldo Anggaran Lebih Akhir',
        value: evaluateXPathToNumber('sum(//sal/heading/subheading/account/number())', this.doc),
        previous: evaluateXPathToNumber('sum(//sal/heading/subheading/account/number())', this.doc)
      }]
